window.onload = function () {
    ...
    $.ajax({
        ...
        success: function(res) {
            userId=res.data.id;
            $.ajax({
                url:"http://8.134.104.184:8078/paper/list?type=2",
                type: 'GET',
                async: false,
                headers:{
                    session:sessionId,
...
function search(){
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            {    
                var prg= prgList[i];  
                html+= '<section class="article-item zoomIn article"><div class="fc-flag">'+judgeStatus(paper.status)+'</div><h5 class="title"><span class="fc-blue">'+prgJudge(prg.type)+'</span>'
